...
<head>
    ...
    <script>
        ...
        function validateFurigana() {
            ...
            const katakanaRegex = /^[ァ-ヶー　]+$/; // 全角カタカナとスペース
...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm'])) {
        ...
        // サーバー側の全角カタカナチェック
        if (!preg_match("/^[ァ-ヶー　]+$/u", $furigana)) { // 全角カタカナとスペースのみ
        ...
        <form method="POST" action="index.php" enctype="multipart/form-data" onsubmit="return validateFurigana()">
            <div class="form-container">
                <fieldset>
                    ...
                    <label>名前：<input type="text" name="name" value="<?= $name ?>" required></label><br>
                    <label>フリガナ：<input type="text" name="furigana" id="furigana" value="<?= $furigana ?>" required></label><br>
                    <label>ニックネーム：<input type="text" name="nickname" value="<?= $nickname ?>"></label><br>
                    <label>Email：<input type="email" name="email" value="<?= $email ?>" required></label><br>
                    <label>自己紹介文：<textarea name="introduction" rows="4" cols="40" required><?= $introduction ?></textarea></label><br>
                    <label>趣味・特技：<textarea name="hobby_skill" rows="2" cols="40"><?= $hobby_skill ?></textarea></label><br>
                    <label>一言メッセージ：<textarea name="message" rows="2" cols="40"><?= $message ?></textarea></label><br>
        ...
        <form method="POST" action="index.php" enctype="multipart/form-data" onsubmit="return validateFurigana()">
            <div class="form-container">
                <fieldset>
                    ...
                    <label>フリガナ：<input type="text" name="furigana" id="furigana" required></label><br>
                    <label>ニックネーム：<input type="text" name="nickname"></label><br>
                    <label>Email：<input type="email" name="email" required></label><br>
                    <label>自己紹介文：<textarea name="introduction" rows="4" cols="40" required></textarea></label><br>
                    <label>趣味・特技：<textarea name="hobby_skill" rows="2" cols="40"></textarea></label><br>
                    <label>一言メッセージ：<textarea name="message" rows="2" cols="40"></textarea></label><br>
                    <label>プロフィール画像：<input type="file" name="profile_image"></label><br>
                    <button type="submit" name="confirm" class="btn btn-primary">確認画面へ</button>
                </fieldset>
            </div>
